...
```{r DataExercise-1a}
vectorhigh <- c(93,93,73,78,85,82,78,75,73,71)
vectorlow <- c(65,53,48,59,61,54,51,51,52,55)
...
```{r DataExercise-1b}
celhigh <- ((vectorhigh - 32)*5)/9
celhigh
cellow <- ((vectorlow - 32)*5)/9
cellow
```
...
c <- c(celhigh)
d <- c(cellow)
dataframeweather <- data.frame(a,b,c,d) 
names(dataframeweather) <- c("farenheit h","farenheit l","celsius h", "celsius l"); View(dataframeweather)
```
...
```{r DataExercise-1d}
summary(dataframeweather)
lapply(dataframeweather, sd)
